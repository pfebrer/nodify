...
def test_plot_connected():
    ...
    assert hasattr(
        plot, "socketio"
    ), f"Socketio connectivity is not initialized correctly in {plot.__class__}"
...
def test_session_connected():
    ...
    assert (
        new_plot.socketio == 2
    ), f"Socketio not transfered from {session.__class__} to plot on add_plot"
    # Fake a disconnection of the session and see if the plot follows
    session.socketio = None
    assert (
        new_plot.socketio is None
    ), f"Socketio change in {session.__class__} not transmitted to plots"
