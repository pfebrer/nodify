...
def node_to_python_script(output_node: Node, include_defaults: bool = False, as_function: bool = False, function_name: Optional[str] = "function", names_map: Dict[int, str] = {}):
    ...
    def build_script(variables, include_defaults: bool = False, as_function: bool = False, function_name: Optional[str] = "function"):
        ...
        for var in variables.values():
            ...
            module = var["module"]
            name = var["name"]
            if name in variable_names or (name in function_modules and function_modules[name] != module):
                name = None
        ...
        if as_function:
            ...
            script += ",".join([f"{const['key']}={repr(const['value'])}" for const in constants])
            script += "):\n"
    ...
    script = build_script(variables, include_defaults=include_defaults, as_function=as_function, function_name=function_name)
...
def python_script_to_nodes(code: str) -> Tuple[List[Node], Dict[int, str]]:
    ...
    ns = {}
    code = nodify_code(code, namespace=ns, assign_fn=assign_fn, nodify_constant_assignments=True)
