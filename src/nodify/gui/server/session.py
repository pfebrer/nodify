...
from nodify import Node, Workflow, nodify_module, ConstantNode
from nodify.utils import traverse_tree_forward, traverse_tree_backward, visit_all_connected
...
from ..registry import SessionRegistry, GUI_REGISTRY
...
class Session:
    ...
    def __init__(self, *args, **kwargs):
        ...
        GUI_REGISTRY.subscribe(self.registry)
    ...
    def init_node(self, cls: int, kwargs={}, input_modes={}, name: Union[str, None] = None) -> Node:
        ...
        """
        node_class = GUI_REGISTRY.classes_dict[cls]
    ...
    def node_to_python_script(self, key: Union[str, int], include_defaults: bool = False, as_function: bool = False, function_name: Optional[str] = "function") -> str:
        ...
        return node_to_python_script(node, include_defaults=include_defaults, as_function=as_function, function_name=function_name, names_map=names_map)
    ...
    def get_compatible_following_nodes(self, node_key: Union[str, int], return_id: bool = False) -> List[Union[Type[Node], int]]:
        ...
        nodes = GUI_REGISTRY.types_registry[out_class]["first_arg"]
    ...
    def to_json(self):
        ...
        # # for cls in GUI_REGISTRY.all_classes:
        # #     try:
        # #         encoder.encode(cls)
        ...
        return dict(
            ...
            nodes=self.nodes,
            node_classes=GUI_REGISTRY.classes_dict,
    ...
    def saves(self, as_string: bool = False) -> Union[str, dict]:  
        ...
        for node_id, node_spec in self.nodes.items():
            ...
            node_to_save = {
                ...
                "inputs": {k: inputs[k] for k in node.__signature__.parameters if k in inputs},
    ...
    def loads(self, state: Union[dict, str]):
        ...
        def _load_node(node_to_load, nodes_to_load):
            ...
            if module.startswith("nodified_"):
                mod = module.split(".")[0]
                if mod not in nodified_modules and mod.replace("nodified_", "") in self.allowed_nodifiable:
    ...
    def set_flows(self, flows: Dict) -> float:
        ...
        for flow in flows.values():
            ...
            flow["nodes"] = [node_id for node_id in flow["nodes"] if node_id in self.nodes]
    ...
    def get_node_classes(self) -> Dict:
        return GUI_REGISTRY.node_class_specs
    
    def get_node_classes_registry(self) -> Dict:
        return {
            "node_classes": GUI_REGISTRY.node_class_specs,
            "types_registry": dict(GUI_REGISTRY.types_registry_with_ids),
            "typehints": GUI_REGISTRY.typehint_specs
        }
